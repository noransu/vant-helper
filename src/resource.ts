  ...
  static fixResource(file: string, vs: any): void {
    ...
    Resource.get(htmlPath)
      .then((content: string) => {
        ...
        const fixScript = $(`<script type="text/javascript" src="${Path.join(Resource.RESOURCE_PATH, 'vant', `fix.js`)}"></script>`);
        const style = $(`<link href="${Path.join(Resource.RESOURCE_PATH, 'vant', 'style.css')}" rel="stylesheet">`);
